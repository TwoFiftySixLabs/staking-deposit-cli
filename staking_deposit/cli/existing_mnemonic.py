    ...
    decorators = [
        jit_option(
            ...
            help=lambda: load_text(
                ['arg_mnemonic', 'help'], func='existing_mnemonic'),
...
@jit_option(
    callback=captive_prompt_callback(
        ...
        lambda: load_text(['arg_validator_start_index',
                          'prompt'], func='existing_mnemonic'),
        lambda: load_text(['arg_validator_start_index',
                          'confirm'], func='existing_mnemonic'),
    ),
    default=0,
    help=lambda: load_text(
...
@click.pass_context
def existing_mnemonic(ctx: click.Context, mnemonic: str, mnemonic_password: str, devnet_chain_setting: str, **kwargs: Any) -> None:
    # Create a new ctx.obj if it doesn't exist
    ctx.obj = {} if ctx.obj is None else ctx.obj
    ctx.obj.update(
        {'mnemonic': mnemonic, 'mnemonic_password': mnemonic_password})
